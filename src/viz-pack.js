...
//   events.bin    — one 12-byte record per stored event, sorted (lane, tMs)
...
export async function packSession(inputPath, outDir) {
  ...
  pids.forEach((pid, lane) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      events.writeUInt32LE(tMs, base + 6);
      ...
      } else if (type === TYPE_CODES.noteOff || type === TYPE_CODES.disconnect) {
        const prev = open.get(finger);
        if (prev) {
  ...
  strokes.forEach((st, i) => {
    ...
    strokeBuf.writeUInt32LE(st.t0, base + 4);
    strokeBuf.writeUInt32LE(Math.min(st.t1, durationMs), base + 8);
